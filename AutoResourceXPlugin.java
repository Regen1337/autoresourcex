...
public class AutoResourceXPlugin extends Plugin
{
    ...
    // subscribe to GrandExchangeOfferChanged event
    @Subscribe
    public void onGrandExchangeOfferChanged(GrandExchangeOfferChanged event)
    {
        if (event.getOffer().getState() == GrandExchangeOfferState.BOUGHT)
        {
            int itemId = event.getOffer().getItemId();
            int price = event.getOffer().getPrice();
            int quantity = event.getOffer().getTotalQuantity();
            int totalPrice = price * quantity;
            log.info("Bought " + quantity + " " + itemManager.getItemComposition(itemId).getName() + " for " + totalPrice + " gp");
            geAddItemToBuyList(itemId, quantity);
        ...
        else // ran every tick inbetween top 2
        {
            switch (currentState) {
                ...
                case ALCHING:
                    ...
                    break;
                case SCANNING_FOR_MINEABLE:
                    if (isMining() || client.getLocalPlayer().getInteracting() == null || (client.getLocalPlayer().getInteracting() instanceof TileObject && !isValidRock(client.getLocalPlayer().getInteracting().getId()) || getMineable() == null)) {
                        currentState.setIsComplete(true);
                    }
                    break;
                case SCANNING_FOR_CHOPABLE:
                    if (isChopping() || client.getLocalPlayer().getInteracting() == null) {
                        currentState.setIsComplete(true);
                    }
                    break;
                case SCANNING_FOR_FIRE:
                ...
                case BANKING:
                    ...
                    if (lastTime == null || Instant.now().isAfter(lastTime))
                    {
                        lastTime = Instant.now().plusSeconds(5);
                        if (config.restockRunes() && shouldRestockRunes() && !isBuying)
                        {
                        ...
                        else if (config.restockItems() && shouldRestockItems() && !isBuying)
                        {
    ...
    public boolean canBuy(int itemId, int amount, int price)
    {
        ...
        {
            log.info("GE Limit Reached for " + itemId);
            geSetDelay(itemId);
            return false;
        }
        log.info("Max Can Buy:" + (geLimit - geGetAmountInBuyList(itemId)));
    ...
    private int geGetAmountInBuyList(int itemID) {
        return configManager.getRSProfileConfiguration(CONFIG_GROUP, CONFIG_BOUGHT + itemID) != null ? configManager.getRSProfileConfiguration(CONFIG_GROUP, CONFIG_BOUGHT + itemID, int.class) : 0;
    }
    ...
    private void geAddItemToBuyList(int itemID, int amount) {
        if (geIsInBuyList(itemID)) {
            ...
            configManager.setRSProfileConfiguration(CONFIG_GROUP, CONFIG_BOUGHT + itemID, currentAmount + amount);
        } else {
            configManager.setRSProfileConfiguration(CONFIG_GROUP, CONFIG_BOUGHT + itemID, amount);
        }
    }
    ...
    public boolean highAlch()
    {
        ...
        {  
            if (alchItem.isNoted()) 
            {
                alchingItem = alchItem.getNotedId();
            }
            else
            {
